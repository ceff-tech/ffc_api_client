...
4. **Save Your Token**: One recommended option is to save your token is with the [{`usethis`}](https://usethis.r-lib.org/) package. To do so, install the package and then run `usethis::edit_r_environ()` to edit your `.Renviron`. Add your token as shown below, make sure there is an empty line at the bottom of the file, and save the file.
    ...
    ```
    Once you have saved your token in your `.Renviron` file, restart R, and you can access it via: `ffctoken <- set_token(Sys.getenv("EFLOWS", ""))` and use `ffctoken` instead of pasting your full key.
...
## Additional FFC Options 

There are many options that are available for the user to specify within the **`ffcProcessor`** object. These should be called prior to the `setup` and `run` functions. Here are a few options to be aware of:
...
# for example, set minimum years required to 2 and start year to 1980
ffc <- FFCProcessor$new()
